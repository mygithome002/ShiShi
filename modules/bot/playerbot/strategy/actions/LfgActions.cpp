...
bool LfgAcceptAction::Execute(Event event)
{
    ...
    if (id)
    {
        ...
        sLog->outBasic("Bot %s updated proposal %d", bot->GetName().c_str(), id);
...
bool BGJoinAction::JoinProposal()
{
	...
	{
		GroupQueueInfo* ginfo = bgQueue.AddGroup(bot, NULL, bracketEntry, false, false, 0, 0, 0);
		uint32 avgTime = bgQueue.GetAverageQueueWaitTime(ginfo);
		// already checked if queueSlot is valid, now just get it
		uint32 queueSlot = bot->AddBattlegroundQueueId(bgQueueTypeId);
		
		sLog->outDetail("Battleground: player joined queue for bg queue type %u bg type %u: GUID %u, NAME %s",
			bgQueueTypeId, bgTypeId, bot->GetGUID(), bot->GetName().c_str());
		return true;
	}
	return false;
}
...
bool BGStatusAction::Execute(Event event)
{
	...
	{		
		sLog->outDetail("bg.battleground", "Battleground: player (bot) %s (%u) is going to leave battleground (%u).", bot->GetName().c_str(), bot->GetGUID(), battleId);
		bot->LeaveBattleground();
		ai->ResetStrategies();
	}
	if (statusid == STATUS_WAIT_QUEUE) //bot is in queue
	{
		sLog->outDetail("bg.battleground", "Battleground: player (bot) %s (%u) is still in queue (%u).", bot->GetName().c_str(), bot->GetGUID(), battleId);
	}
	if (statusid == STATUS_WAIT_JOIN) //bot may join
	{
		if (bot->InBattleground())
		{
			sLog->outDetail("bg.battleground", "Battleground: player (bot) %s (%u) is in battleground already. (%u, %u).", bot->GetName().c_str(), bot->GetGUID(), battleId, instanceId);
			return false;
		}

		sLog->outDetail("bg.battleground", "Battleground: player (bot) %s (%u) received a invite to a battleground (%u, %u).", bot->GetName().c_str(), bot->GetGUID(), battleId, instanceId);

		bot->CombatStop(true);

		sLog->outBasic("Bot %s is going to join BG %d", bot->GetName().c_str(), battleId);
		...
		{
			sLog->outBasic("Bot %s was not in queue to join BG %d", bot->GetName().c_str(), battleId);
			return false;
		}
		// if action == 1, then instanceId is required
		if (!ginfo.IsInvitedToBGInstanceGUID)
		{
			sLog->outBasic("Bot %s had no instanceID to join BG %d", bot->GetName().c_str(), battleId);
			return false;
		}
		...
		if (!bg)
		{
			...
			if (!bg)
			{
				sLog->outBasic("Bot %s could find no template for Battleground %d", bot->GetName().c_str(), battleId);
				return false;
			}
		}
		...
		if (!bracketEntry)
		{
			sLog->outBasic("Bot %s has no bracket (%d) available to join BG %d", bot->GetName().c_str(), bot->getLevel(), battleId);
			return false;
		}
		...
		if (ginfo.ArenaType == 0)
		{
			...
			if (!bot->CanJoinToBattleground())
			{
				sLog->outBasic("Bot %s cant join BG %d because it has the deserter debuf", bot->GetName().c_str(), battleId);
				return false;
			}
			//if player don't match battleground max level, then do not allow him to enter! (this might happen when player leveled up during his waiting in queue
			if (bot->getLevel() > bg->GetMaxLevel())
			{				
				sLog->outBasic("Bot %s cant join BG %d because its level does not fit", bot->GetName().c_str(), battleId);
				return false;
			}
		}
		...
		if (bot->HasAura(9454))
		{
			sLog->outBasic("Bot %s cant join BG %d because it is freezed", bot->GetName().c_str(), battleId);
			return false;
		}

		if (bot->GetBattlegroundQueueIndex(bgQueueTypeId) == PLAYER_MAX_BATTLEGROUND_QUEUES)
		{
			sLog->outBasic("Bot %s cant join BG %d because it was not invited to join", bot->GetName().c_str(), battleId);
			return false;                                 // cheating? No, bots dont cheat. They just try very hard.
		}
		else if (!bot->IsInvitedForBattlegroundInstance())
		{
			sLog->outBasic("Bot %s cant join BG %d because it was not invited to join", bot->GetName().c_str(), battleId);
			return false;                                 // cheating? No, bots dont cheat. They just try very hard.
		}
		...
		bgQueue.RemovePlayer(bot->GetGUID(), false, 0);
		...
		bot->SetBattlegroundId(bg->GetInstanceID(), bgTypeId, 0, false, false, ginfo.teamId);
		...
		sLog->outDetail("bg.battleground", "Battleground: player (bot) %s (%u) joined battle for bg %u, bgtype %u, queue type %u.", bot->GetName().c_str(), bot->GetGUID(), bg->GetInstanceID(), bg->GetBgTypeID(), bgQueueTypeId);
		sLog->outBasic("Bot %s joined BG %d", bot->GetName().c_str(), battleId);
	}
	return true;
}
...
bool BGTacticsWS::consumeHealthy(Battleground *bg)
{
	...
	{
		Position posH1 = { 1111.526733f, 1352.458130f, 316.567047f, 0 };
		Position posA1 = { 1320.670288f, 1549.642700f, 313.501404f, 0 };
...
bool BGTacticsWS::moveTowardsEnemyFlag(BattlegroundWS *bg)
{
	...
	if (!(bg->GetFlagState(bg->GetOtherTeamId(bot->GetTeamId())) == BG_WS_FLAG_STATE_ON_BASE ||
		bg->GetFlagState(bg->GetOtherTeamId(bot->GetTeamId())) == BG_WS_FLAG_STATE_ON_GROUND))
		return false;
	
	GameObject* target_obj = bg->GetBgMap()->GetGameObject(bg->GetDroppedFlagGUID(bg->GetOtherTeamId(bot->GetTeamId()) == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE));
	if (target_obj == NULL)
	{
...
bool BGTacticsWS::homerun(BattlegroundWS *bg)
{
	...
	{
		GameObject* obj = bg->GetBgMap()->GetGameObject(bg->GetFlagPickerGUID(bot->GetTeamId() == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE));
		if (obj == NULL)
		{
			if (bot->GetTeamId() == TeamId::TEAM_HORDE)
				obj = bg->GetBgMap()->GetGameObject(bg->BgObjects[BG_WS_OBJECT_H_FLAG]);
		...
		if (bot->IsWithinDistInMap(obj, 10) && (bg->GetFlagState(bot->GetTeamId()) == BG_WS_FLAG_STATE_ON_BASE))
		{
		...
		if (supporter || (bg->GetFlagState(bot->GetTeamId()) != BG_WS_FLAG_STATE_ON_PLAYER))
		{
			uint64 guid = bg->GetFlagPickerGUID(bg->GetOtherTeamId(bot->GetTeamId()) == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);
			...
			uint64 guid = bg->GetFlagPickerGUID(bot->GetTeamId() == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);
...
bool BGTacticsWS::Execute(Event event)
{
	...
	//Check for Warsong.
	if (bot->GetBattleground()->GetBgTypeID() == BattlegroundTypeId::BATTLEGROUND_WS)
	{
		ai->SetMaster(NULL);
		...
		if (bg != NULL && !bot->isDead())
		{
			...
			bool alreadyHasFlag = bg->GetFlagState(bg->GetOtherTeamId(bot->GetTeamId())) == BG_WS_FLAG_STATE_ON_PLAYER;
			if (!alreadyHasFlag)
			{
				GameObject* target_obj = bg->GetBgMap()->GetGameObject(bg->GetDroppedFlagGUID(bg->GetOtherTeamId(bot->GetTeamId()) == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE));
				if (target_obj == NULL)
			...
			{
				if (bot->GetTeamId() == TEAM_ALLIANCE) //you.. may... NOT.. PASS!
				{
				...
				{
					float x, y, z, o;
					bg->GetTeamStartLoc(bot->GetBgTeamId(), x, y, z, o);
					for (std::map<uint64, Player*>::const_iterator itr = bg->GetPlayers().begin(); itr != bg->GetPlayers().end(); ++itr)
					{
						if (urand(0, 100) < 2)
