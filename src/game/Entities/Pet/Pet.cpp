...
{ 
    ...
    else
    {
		...
		PreparedQueryResult checkResult = CharacterDatabase.Query(stmt);
		// EJ debug
		WorldSession* destSession = owner->GetSession();

		destSession->HandleLoadPetFromDBFirstCallback(checkResult, asynchLoadType);
    }
...
void Pet::SavePetToDB(PetSaveMode mode, bool logout)
{
    ...
    CharacterDatabase.DirectCommitTransaction(trans); /*update to direct*/
...
{ 
    ...
    if (!spellInfo)
    {
        ...
        if (state == PETSPELL_UNCHANGED)                    // spell load case
        {
            ...
            CharacterDatabase.DirectExecute(stmt); /*update to direct*/
        }
        else
